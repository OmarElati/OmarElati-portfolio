...
// === DYNAMIC FETCH (for node-fetch@2.6.7 in ESM) ===
let fetchFn;
...
// === MAIN FUNCTION ===
export const fetchRepos = async () => {
  const fetch = await getFetch();  // ← Critical: await the fetch
  ...
  // === ALL YOUR HELPERS BELOW (unchanged) ===
  const fetchImage = async (repoName) => {
    const imageExtensions = ['jpg', 'jpeg', 'png', 'gif', 'webp'];
    try {
      const contentResponse = await fetch(
      ...
      console.error(`Image error ${repoName}:`, error.message);
      return `https://opengraph.githubassets.com/1/OmarElati/${repoName}`;
    }
  };
  ...
  const classifyRepo = async (repo) => {
    try {
      ...
      const rules = {
        frontend: { files: ['index.html', 'style.css'], keywords: ['react', 'vue', 'ui'] },
        backend: { files: ['server.js', 'app.py'], keywords: ['node', 'django', 'api'] },
        fullstack: { keywords: ['fullstack', 'mern', 'nextjs'] },
        mobile: { files: ['pubspec.yaml'], keywords: ['flutter', 'react-native'] },
        dataScience: { files: ['notebook.ipynb'], keywords: ['ml', 'ai'] },
        devops: { files: ['dockerfile'], keywords: ['docker', 'ci-cd'] },
      };

      for (const [cat, { files: f = [], keywords = [] }] of Object.entries(rules)) {
        const match =
          f.some(x => files.includes(x)) ||
          keywords.some(k => name.includes(k) || desc.includes(k));
        if (match) return cat;
